...
def make_iformat_reader_class():
    ...
    class IFormatReader(object):
        ...
        ##### added #####
        getEffectiveSizeC = jutil.make_method('getEffectiveSizeC',
                                              '()I','Gets the effective size of the C dimension, guaranteeing that getEffectiveSizeC() * getSizeZ() * getSizeT() == getImageCount() regardless of the result of isRGB().')

        getBitsPerPixel = jutil.make_method('getBitsPerPixel',
                                            '()I','Gets the number of valid bits per pixel. The number of valid bits per pixel is always less than or equal to the number of bits per pixel that correspond to getPixelType().')
        #################
