...
from mainApi.app.images.utils.file import add_image_tiles, convol2D_processing, convol3D_processing
...
async def upload_image_tiles(files: List[UploadFile] = File(...),
    ...
    current_image_array = io.imread(file_path)
...
async def upload_image_name(files_name: str = File(...),
    ...
    result = convol2D_processing(current_image_array, STATIC_PATH)
    return JSONResponse(result)

@router.post("/convol3D/", 
        response_description="Convolution about 3D image",
        status_code=status.HTTP_201_CREATED,
        response_model = List[TileModelDB])
async def upload_image_name(files_name: str = File(...),
                    clear_previous: bool = Form(False),
    ...
    global current_image_array
    files_name = files_name.split("/")[-1]
    result = convol3D_processing(current_image_array, STATIC_PATH)
    return JSONResponse(result)
